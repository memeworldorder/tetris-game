/* eslint-disable */
// switchboard-vrf-client.ts – migrated from monolith
...
export class SwitchboardVRFClient {
  ...
  constructor(
    ...
    this.programId =
      programId || new PublicKey('SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f')
    this.vrfAccount = Keypair.generate()
    // Use test queue if not provided
    this.queueAccount =
      queueAccount || new PublicKey('A43DyUGA7s8eXPxqEjJY6EBu1KKbNgfxF8h4ugQFn5s')
  }
  ...
  async requestRandomness(): Promise<{
    ...
    this.vrfAccountData.requestHash = crypto
      .createHash('sha256')
      .update(requestData)
      .digest()
  ...
  async getVRFResult(): Promise<{
    ...
    return {
      ...
      result:
        this.vrfAccountData.status === VRFStatus.StatusVerified
          ? this.vrfAccountData.result
          : null,
      proof:
        this.vrfAccountData.status === VRFStatus.StatusVerified
          ? this.vrfAccountData.proof
          : null,
      counter: this.vrfAccountData.counter,
    }
  }
  ...
  verifyVRFProof(result: Buffer, proof: Buffer, requestHash: Buffer): boolean {
    try {
      ...
      const expectedProofData = Buffer.concat([
        ...
        Buffer.from(
          this.vrfAccountData?.requestSlot.toString().padStart(16, '0') || '0',
        ), // 16 bytes
      ])
      const expectedProof = crypto.createHash('sha256')
        .update(expectedProofData)
        .digest()
  ...
  getEstimatedCost(): number {
    const queueReward =
      this.queueAccountData?.reward || 0.000025 * LAMPORTS_PER_SOL
  ...
  async getRaffleResult(maxWaitMs: number = 10000): Promise<Buffer | null> {
    ...
    while (Date.now() - startTime < maxWaitMs) {
      ...
      if (result.status === VRFStatus.StatusVerified && result.result) {
        ...
        if (
          vrfInfo &&
          this.vrfClient.verifyVRFProof(result.result, result.proof!, vrfInfo.requestHash)
        ) {
